...
const Header = () => {
    ...
    return (
        ...
        <header className='bg-black w-full h-auto fixed top-0 shadow-lg p-2 px-8 flex justify-between items-center z-10 '>
            <Link to='/'>
                <div className='logo bg-[#1c1c24] rounded-xl ml-2 px-4 flex items-center'>
                    <img src={thirdweb} className='w-14 h-14' alt='logo' />
                </div>
            </Link>
            <div className="wallet flex items-center gap-8">
                ...
                {profileData && profileData?.owner ?
                    <Link to="/profile" className="profile rounded-full bg-[#1c1c24]  ">
